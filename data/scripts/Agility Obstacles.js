...
function varrockWallClimb(player, obstacle, object) {
    ...
    Agility.forceTeleport(player, Animation.create(737), Location.create(3221, 3414, 1), 0, 2);
...
function varrockClohtesLine(player, obstacle, object) {
	if(player.getLocation().getY() != 3214) {
		player.removeAttribute("busy");
		return;
	}
	var varrockAgilityCourseLvl = player.getAttribute("varrockAgilityCourse");
	...
	Agility.setRunningToggled(player, false, 12);
	Agility.forceWalkingQueue(player, Animation.create(762), 3208, 3414, 3, 10, false);
	player.getSkills().addExperience(Skills.AGILITY, obstacle.getExperience());
}
