...
public class MonsterExamineServiceImpl implements MonsterExamineService {
	...
	public void openMonsterExamine(@Nonnull Player player, int npcId) {
		...
		player.getActionSender().sendString(WIDGET, 2, "<col=800000>" + CacheNPCDefinition.get(npcId).getName());
		int child = 4;
		for (int i = 0; i < rarities.size(); i++) {
			...
			player.getActionSender().sendString(WIDGET, child++, name + ":  Amount: [" + rarity.getMinAmount() + "-" + rarity.getMaxAmount() + "]:              " + rarity.getColor() + rarity.toString());
		}
