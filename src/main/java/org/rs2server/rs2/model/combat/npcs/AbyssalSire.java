...
public class AbyssalSire extends AbstractCombatAction {
	...
	public void hit(final Mob attacker, final Mob victim) {
		...
		case MELEE:
			...
			randomHit = super.damage(maxHit, attacker, victim, AttackType.CRUSH, Skills.ATTACK, Prayers.PROTECT_FROM_MELEE, false, true);
			...
			for(final Player near : localPlayers) {
				if(near != null && near != attacker && near.getSkills().getLevel(Skills.HITPOINTS) > 0) {
					if (attacker.getCentreLocation().isWithinDistance(attacker, near, 10)) {
						...
						attacker.playProjectile(Projectile.create(attacker.getCentreLocation(), near.getCentreLocation(), 1275, 45, 50, rClientSpeed, 43, 35, near.getProjectileLockonIndex(), 10, 48));
					}
