...
public class WarriorsGuild {
	...
	public void killedCyclop(Location pos) {
		...
		if(IN_GAME.contains(player)) {
			if(r.nextInt(5) == 0 && defenderDrop != null) {
				player.getActionSender().sendDialogue("", DialogueType.MESSAGE, -1, null, "You've received a defender, After picking it up leave and come back in to<br>work towards the next one!");
