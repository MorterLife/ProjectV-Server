			...
			if (loot != null) {
				...
				if (pets != null) {
					if (challenger.getPet() != null) {
						...
						World.getWorld().sendWorldMessage("<col=880000><img=33>Drops: <col=880000>" + challenger.getName() + " has just received " + item.getCount() + "x " + item.getDefinition2().getName() + ".");
						continue;
					}
				}
				...
				if (def != null && def.getName() != null) {
					...
					if (loot.getHitRollCeil() <= 3) {
						World.getWorld().sendWorldMessage("<col=880000><img=33>Drops: <col=880000>" + challenger.getName() + " has just received " + g.getCount() + "x " + name + ".");
					} else {
						challenger.getActionSender().sendMessage("<col=880000><img=33>Drops: <col=880000>" + challenger.getName() + " has just received " + g.getCount() + "x " + name + ".");
					}
				}
