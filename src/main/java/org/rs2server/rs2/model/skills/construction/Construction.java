...
public class Construction {
	...
	/**
	 * All the non-transparent objects such as walls, grass,
	 * etc should go in here.
	 */
	@SuppressWarnings("unused")
	private static final int[] NON_HOTSPOT_OBJECTS = {
		13410, //Grass.
	};
			...
			for(File file : files) {
				...
				while (dis.available() != 0) {
					...
					try {
						...
						//p.println(objectId + "	" + objX + "	" + objY + "	" + plane + "	" + objectType + "	" + objectRotation);
						...
						ConstructionObject object = new ConstructionObject(GameObjectDefinition.forId(id), loc, type, face, hotspot);
						if(hotspot) {
	...
	/**
	 * Called every time a player attempts
	 * to build an object.
	 */
	...
	/**
	 * Switches the current construction mode..
	 * Building mode on /off..
	 * @param on 
	 */
		...
		if(inBuildingMode != on) {
			if(CONSTRUCTORS.contains(player)) {
				if(house.hasVisiters()) {
					...
					player.getActionSender().sendMessage("Building mode is now " + (inBuildingMode ? "on" : "off") + ".");
					return true;
				}
			} else {
	...
	/**
	 * Called when attempting to enter our house.
	 * Example usage: Teleporting, portal.
	 * @param buildingMode Defines if we enter the house in Building Mode.
	 */
	public void enterHouse(boolean buildingMode) {
		inBuildingMode = buildingMode;
		house.enter(Location.create(1955, 5732, 0)); //TODO: New location for each constructor.
	}
		...
		case 402:
			...
			if(roomType != null) {
				if(player.getSkills().getLevelForExperience(Skills.CONSTRUCTION) < roomType.getLevel()) {
					player.getActionSender().sendMessage("You need a Construction level of " + roomType.getLevel() + " in order to build this room.");
					return true;
				}
				...
				Room room = new Room(roomType, player, roomType.getPaletteTile(house.getHouseDesign()), buildingLocation.getX(), buildingLocation.getY(), buildingLocation.getZ());
				...
				for(GameObject hotspot : buildingRoom.getHotspots()) {
					Location local = Location.create(hotspot.getLocation().getX() + buildingRoom.getX(), hotspot.getLocation().getY() + buildingRoom.getY(), hotspot.getLocation().getZ());
	...
	public void spawnRoomForRotation(boolean clockwise) {
		...
		for(GameObject hotspot : buildingRoom.getHotspots(player, clockwise)) {
			Location local = Location.create(hotspot.getLocation().getX() + buildingRoom.getX(), hotspot.getLocation().getY() + buildingRoom.getY(), hotspot.getLocation().getZ());
			//player.getActionSender().sendCreateLocalObject(hotspot.getDefinition().getId(), hotspot.getType(), hotspot.getDirection(), local);
		}
	}

	/**
	 * Gets the hotspots for a specific tile set.
	 * @param tile The palette tile to get hotspots for.
	 * @return An object[] containing all the hotspots.
	 */
	public static ConstructionObject[] loadHotspots(int z, PaletteTile tile) {
		/*
		 * I had to add this, to make sure our definitions
		 * are all loaded before we start getting content from it.
		 * (Player loading)
		 */
		while(!loaded) {
			try {
	...
	public static ConstructionObject[] getOtherObjects(Player player, PaletteTile tile) {
		...
		for(ConstructionObject obj : objs) {
			...
			case 13830: //Stupid transparent window
				...
				case BASIC_WOOD:
					objects.add(new ConstructionObject(GameObjectDefinition.forId(13730), obj.getLocation(), obj.getType(), obj.getDirection(), true));
					objects.add(new ConstructionObject(GameObjectDefinition.forId(13730), obj.getLocation(), obj.getType(), obj.getDirection(), false)); //TODO: Real id.
					//id = 13730; //TODO: This is building mode objects LOL
					break;
				case BASIC_STONE:
					objects.add(new ConstructionObject(GameObjectDefinition.forId(13728), obj.getLocation(), obj.getType(), obj.getDirection(), true));
					objects.add(new ConstructionObject(GameObjectDefinition.forId(13728), obj.getLocation(), obj.getType(), obj.getDirection(), false)); //TODO: Real id.
					//id = 13728;
					break;
				}
				break;
			...
			case 15322:	//Door hotspot
				...
				id2 = 15644;
				//} else {
				//id2 = 15645; //Those are wrong, but it looks fine for now. //Open door lol
				//}
				objects.add(new ConstructionObject(GameObjectDefinition.forId(id2), obj.getLocation(), obj.getType(), obj.getDirection(), false));
	...
	public static void enterFriendsHouse(Player player, long name) {
		...
		//for(long n : player.getPrivateChat().getFriends()) {
			...
			//	for(Player p : World.getWorld().getPlayers()) {
				//	if(p.getNameAsLong() == name) {
					//	if(CONSTRUCTORS.contains(p)) {
						...
						//		player.getActionSender().sendMessage(string + " doesn't want visitors at the moment.");
							//} else if(p.getConstruction().isInBuildingMode()) {
							//	player.getActionSender().sendMessage(string + " is currently in building mode.");
							//} else {
							//	p.getConstruction().getHouse().visitor(player);
							//}
						//} else {
							//player.getActionSender().sendMessage(string + " is not home at the moment. Try again later.");
					//	}
					//	return;
					//}
				//}
			...
			//}
		//}
		//player.getActionSender().sendMessage("Unable to find the player: " + string + ". Please try again.");
	}
	...
	public enum BuildingType {
		...
		SMALL_TREES(396, new String[][] {
				...
				10, //'Yew tree'
				10, //'Magic tree'
		}, new double[] { 31, 44, 70, 100, 122, 141, 223 }
		, new int[] {2293, 2293, 2293, 2293, 2293, 2293, 2293}
		), //watering can.
		...
		BIG_TREES(396, new String[][] {
				...
				{"Level 60", "Yew tree", "1 Bagged yew tree", "Watering can", "", ""},
				{"Level 75", "Magic tree", "1 Bagged magic tree", "Watering can", "", ""},
		}, new Item[]{ //The items to put on the interface. Seems kinda weird that they did items for this O,o..
				new Item(8173),  //Tree
		...
		}, new Item[][] { //Item requirements.. Notice: Watering cans are hardcoded.
				{new Item(8417)}, //Bagged dead tree
				{new Item(8419)}, //Bagged nice tree
				{new Item(8421)}, //Bagged oak tree
				{new Item(8423)}, //Bagged willow tree
		...
		, 
		new int[] {2293, 2293, 2293, 2293, 2293, 2293, 2293}//watering can.
		),

		BOOK_CASE(394, new String[][] {
		 ...
		 * @param interfaceId The interface ID.
		 */
		private BuildingType(int interfaceId, String[][] messages, Item[] items, Item[][] requirements, int[] objects, int[] objectTypes, double[] exp, int[] animations) {
			this.interfaceId = interfaceId;
			...
			case 394:
				...
				for(int i = 0; i < childs2.length; i++) {
					for(int index = 0; index < childs2[i].length; index++) {						
						player.getActionSender().sendBackwardsString(messages[i][index], interfaceId, childs2[i][index]);
					}
				}
		...
		public static BuildingType getBuildingType(RoomType type, int objectId) {
			...
			default: System.out.println("Unhandled building object: " + objectId);
			}
			switch(type) {
			case GARDEN:
				switch(objectId) {
			...
			for(index = 0; index < items.length; index++) {
				if(items[index].getId() == itemId) {
					...
					if(player.getSkills().getLevel(Skills.CONSTRUCTION) < levelReq) {
						player.getActionSender().sendMessage("You need a Construction level of level " + levelReq + " in order to build this object.");
						return true;
					}
					for(Item item : requirements[index]) {
						if(item.getId() != -1) { //Nail stuff..
							if(!player.getInventory().containsItems(nails)) {
								player.getActionSender().sendMessage("You need " + item.getCount() + " " + item.getDefinition().getName().toLowerCase() +(item.getCount() > 1 ? "s" : "")+" in order to build this object.");
								found = false;
							}
						}
					}
					...
					public void execute() {
						...
						if(firstRound) {
							if(garden) {
								...
								if(!found) {
									player.getActionSender().sendMessage("You need a watering can with water in order to build this object.");
								}
							} else {
								//Check if we're using planks building the current object..
								Item[] required = requirements[fIndex];
								for(Item item : required) {
									if(item.getId() == 960) {//regular plank
										for(Item i : required) {
											if(i.getId() == -1) { //We found or nail requirements..
												nailCheck = true;
												...
												if(!found) {
													player.getActionSender().sendMessage("You don't have enough nails to build this item.");
												}
											}
										}
									}
						...
						if(!nailCheck && !firstRound) {
							...
							for(Item req : required) {
								if(req.getId() != -1) { //This is nails, and they were removed above..
							...
							player.getConstruction().buildingRoom.addObject(new ConstructionObject(GameObjectDefinition.forId(objects[fIndex]), player.getConstruction().buildingLocation, objectTypes[fIndex], player.getConstruction().buildingObject.getDirection(),false), true);
							player.getActionSender().sendMessage("You build a " + GameObjectDefinition.forId(objects[fIndex]).getName().toLowerCase() + ".");
							this.stop();
						} else if(nailCheck) {
							boolean have = false;
							Item[] inventory = player.getInventory().toArray(); 
								...
								for(Item inv : inventory) {
									if(inv != null) {
										if(inv.getDefinition().getName().contains("nails")) {
											if(inv.getId() == nail) {
												have = true;
												if(r.nextInt(nails.length + 2) < index) { //Means the nail didn't break..
													if(--nailAmount == 0) {
														nailCheck = false;
													}
												} else {
	...
	/**
	 * Saves the building variables to use again,
	 * once we click interface buttons..
	 * @param hotspot 
	 */
	...
	public boolean handleObjectPackets(Location loc, int id, int option) {
		...
		if(option == 1) {
			...
			case 15644:
			case 15645:
				Location local = Location.create(loc.getX() - 8 * (player.getLastKnownRegion().getRegionX() - 6), loc.getY() - 8 * (player.getLastKnownRegion().getRegionY() - 6), player.getLocation().getZ());
				...
				if(room != null) {
					...
					if(obj != null) {
						//DoorManager.handleConstructionDoor(player, obj, room, local, id);
					}
				}
				return true;
			}
