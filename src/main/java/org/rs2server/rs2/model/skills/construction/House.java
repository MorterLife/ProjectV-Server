...
public class House {
	...
	public void visitor(Player player) {
		if(visitors.size() > 100) {
			...
			visitors.add(player);
			reload(player, Location.create(1955, 5732, 0)); //TODO: Entrance location.
		}
	...
	public void reload(final Player p, final Location location) {
		...
		//TODO//p.getActionSender().send (minimap blackout here once finished type =2;
        ...
        World.getWorld().submit(new Event(1200) {
			...
			@Override
			public void execute() {
		        player.getActionSender().sendConstructMapRegion(pal, location);//TODO 'value: location' (pal, location)
				World.getWorld().submit(new Event(1200) {
					...
					public void execute() {
						for(Room room : rooms) {
				        	...
				        	for(int index = 0; index < hotspots.length; index++) {
				        		...
				        		if(hotspot.getLocation().getZ() == p.getLocation().getZ()) { 
					        		...
					        		if(player.getConstruction().isInBuildingMode()) {
					        			...
					        			player.getActionSender().sendDestroyLocalObject(hotspot.getType(), hotspot.getDirection(), local);
					        		}
					        		if(object != null) {
					        			System.out.println("Object:" + object);
					        		//TODO//	p.getActionSender().sendCreateLocalObject(object.getDefinition().getId(), object.getType(), object.getDirection(), object.getLocation());
					        		}
				        		}		        		
				        	}
				        	if(room.getRoomType() != RoomType.GARDEN) {
					        	for(ConstructionObject other : room.getOtherObjects()) {
						        	...
						        	//TODO//	p.getActionSender().sendCreateLocalObject(other.getDefinition().getId(), other.getType(), other.getDirection(), local);
					        		}
					        	}
				        	}
						}
	...
	public void buy() {
		if(isHouseBought()) {
			...
			rooms.add(room);
			room.addObject(new ConstructionObject(CacheObjectDefinition.forId(13405), Location.create(51, 51, 0), 10, 0, false), false);
		}
	}
