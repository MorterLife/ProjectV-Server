...
public class Shop {
	...
	public static void open(Player player, int shopId, int stockType) {
		...
		if (shopId == 23) {
			int questFinished = player.getSettings().getBestRFDState();
			if (player.getSettings().getRFDState() == 4) {
				for (int i = 0; i < RFD_GLOVES.length; i++) {
	...
	public static void buyItem(Player player, int slot, int id, int amount) {
		...
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 19) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 4) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 5) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 6) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 7) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
		if (permissionService.isAny(player, PermissionService.PlayerPermissions.IRON_MAN, PermissionService.PlayerPermissions.ULTIMATE_IRON_MAN, PermissionService.PlayerPermissions.HARDCORE_IRON_MAN) && player.getInterfaceState().getOpenShop() == 8) {
			player.getActionSender().sendMessage("Sorry you may not purchase from this shop.");
			return;
		}
	...
	public int getCostValue(Player player, Item item) {
		if (player.getInterfaceState().getOpenShop() == 36 || player.getInterfaceState().getOpenShop() == 37) {
...
//		}
		...
		player.getActionSender().sendMessage(item.getDefinition2().getName() + ": currently costs " + (player.getInterfaceState().getOpenShop() == 36 || player.getInterfaceState().getOpenShop() == 37 ? format.format(getTzhaarBuyValue(item)) : format.format(shop.getCostValue(player, item))) + " " + CacheItemDefinition.get(shop.getCurrency()).getName().toLowerCase() + ".");
	}
	...
	public static void valueItem(Player player, int slot, int id) {
		...
		int finalValue = player.getInterfaceState().getOpenShop() == 36 || player.getInterfaceState().getOpenShop() == 37 ? getTzhaarSellValue(item) : shop.getSellValue(player, item);
		String shopAdd = "";
		...
		player.getActionSender().sendMessage(message ? CacheItemDefinition.get(item.getId()).getName() + ": shop will buy for " + finalValue + " " + CacheItemDefinition.get(shop.getCurrency()).getName().toLowerCase() + " " + shopAdd : "This shop owner has no interest in a " + item.getDefinition2().getName() + ".");
	}
