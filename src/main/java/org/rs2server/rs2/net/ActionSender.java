...
//			player.getDetails().setForumRights(12);
//		}
//		
//		for (String admins : DEVS) {
//            if (player.getName().toLowerCase().equals(admins)) {
//				player.getDetails().setForumRights(15);
...
//        }
       ...
       // updateClickPriority();
        ...
        if (starter) {
            ...
            if (count > Constants.MAX_STARTER_COUNT) {
                ...
                //World.getWorld().sendWorldMessage("<col=ff0000><img=21>Player: " + player.getName() +" has just joined ProjectV!");
                sendGlobalCC();
                updateSplitPrivateChatConfig();
                return this;
            } else {
    ...
    public ActionSender sendGlobalCC() {
        if (World.getWorld().privateIsRegistered("Life")) {
            PrivateChat chat = World.getWorld().getPrivateChat().get("ProjectV");
...
outer:
    ...
    public ActionSender updateQuestText() {
        ...
        sendString(399, 13, "Server Name: ProjectV");//[+] 
...
//
    ...
    public static void init(Player player) {
        player.getActionSender().sendString(
                378,
                15,
                "ProjectV Staff will NEVER email you. "
...
//				sendCS2Script(764, new Object[] { 0, 19268, "ProjectV Economy" }, "sIi");
...
Offset: -1, Length: -1, Set: 0, Interface: 193, Child: 4
     ...
     *
     * @param loc      The new camera location.
